...
UNIX（ユニックス）は、1969年にAT&Tベル研究所でケン・トンプソンとデニス・リッチーらによって開発されたオペレーティングシステム（OS）である。
...
Linux（リナックス）は、1991年にリーナス・トーバルズが発表した**カーネル**（OSの中核部分）である。
...
このコースでは最も一般的な `bash` を中心に、POSIX 準拠の共通機能を紹介する。
